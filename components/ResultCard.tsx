  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="rounded-2xl bg-white border border-neutral-200 p-5 shadow-sm">
        ...
        <h2 className="text-2xl font-bold text-neutral-900 mb-2">{faceShapeLabel}</h2>
        <p className="text-sm text-neutral-600 leading-relaxed">{faceShapeDesc}</p>
      </div>
      ...
      {colorAdvice && (
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4">
          ...
          <p className="text-sm text-amber-900 leading-relaxed">{colorAdvice}</p>
        </div>
      )}
...
const accentClasses = {
  ...
  violet: {
    ...
    tip: "bg-violet-100 text-violet-800",
  },
  pink: {
    bg: "bg-pink-50",
...
function RecommendationBlock({ category, accent, name, description, tip }: BlockProps) {
  ...
  return (
    <div className={`rounded-2xl ${cls.bg} border ${cls.border} p-5`}>
      ...
      <p className="text-sm text-neutral-700 leading-relaxed mb-3">{description}</p>
